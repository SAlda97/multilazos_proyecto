# core/views_categoria_gastos.py
import json
...
@csrf_exempt
def categoria_gastos_list(request):
    """
    GET  /categorias-gastos/?search=&page=&page_size=
    POST /categorias-gastos/ { "nombre_categoria": "Servicios" }
    """
    if request.method == "GET":
        ...
        data = {
            ...
            "results": [
                {
                    "id_categoria_gastos": o.id_categoria_gastos,
    ...
    if request.method == "POST":
        ...
        if CategoriaGasto.objects.filter(nombre_categoria__iexact=nombre).exists():
            return JsonResponse({"detail": "Ya existe una categoría con ese nombre."}, status=400)

        obj = CategoriaGasto.objects.create(nombre_categoria=nombre)
        return JsonResponse(
            {"id_categoria_gastos": obj.id_categoria_gastos, "nombre_categoria": obj.nombre_categoria},
            status=201,
        )
...
@csrf_exempt
def categoria_gastos_detail(request, id_categoria_gastos):
    """
    GET    /categorias-gastos/<id>/
    PUT    /categorias-gastos/<id>/   { "nombre_categoria": "..." }
    DELETE /categorias-gastos/<id>/
    """
    try:
        obj = CategoriaGasto.objects.get(pk=id_categoria_gastos)
    except CategoriaGasto.DoesNotExist:
        raise Http404("Categoría de gasto no encontrada")

    if request.method == "GET":
        return JsonResponse({
            "id_categoria_gastos": obj.id_categoria_gastos,
    ...
    if request.method == "PUT":
        ...
        if len(nombre) > 100:
            return JsonResponse({"detail": "El nombre no puede exceder 100 caracteres."}, status=400)
        if CategoriaGasto.objects.filter(
